...
if __name__ == '__main__':
    ...
    alpha = 0.1
    input_ = torch.rand(1, 2, 6000)
    m, f_list = get_prune_paras(alpha=alpha)

    model_0 = LCNet(num_classes=16, n_neuron=32, n_mobileunit=7)
    model_1 = base_LCNet(num_classes=16, n_neuron=32, n_mobileunit=7)  # model_1对应alpha = 0
    model_2 = prune_LCNet(num_classes=16, n_neuron=32, n_mobileunit=7, m=m, f_list=f_list)

    #  1. 获取Params
    Total_params, Params_size = summary(model_1, input_size=(2, 6000), batch_size=1, device="cpu")
    print('alpha =', alpha, '---', 'm =', m)
    print('Total_params =', int(Total_params))
    print('Params_size(MB) = {:.3f}'.format(Params_size))

    #  2. 计算flops
    flops, meters = profile(model_1, inputs=(input_,))
