...
class MCNet(nn.Module):
    ...
    def __init__(self, frame_length=4800, num_classes=16):
        """
...
class base_MCNet(nn.Module):
    ...
    def __init__(self, frame_length=1500, num_classes=16):
        """
        原MCNet网络处理信号长度为1024,wifi数据集信号长度为6000,原MCNet网络在该数据集上欠拟合。
        因此在原MCNet网络前添加两层小卷积核卷积层，将数据维度从6000降到1500。
        """
        super(base_MCNet, self).__init__()
        self.frame_length = frame_length # 信号长度
        self.num_classes = num_classes # 种类

        # 1. 卷积层 x 2 input: 6000 → 1500
        self.pad1 = nn.ZeroPad2d(padding=(1, 1, 1, 1))
        self.conv1 = nn.Conv2d(1, 64, kernel_size=(3, 3), stride=(1, 2))
        self.batchnorm1 = nn.BatchNorm2d(num_features=64)
...
class prune_MCNet(nn.Module):
    ...
    def __init__(self, frame_length=1500, num_classes=16, f_list=None, m=None):
        """
          原MCNet网络处理信号长度为1024,wifi数据集信号长度为6000,原MCNet网络在该数据集上欠拟合。
          因此在原MCNet网络前添加两层小卷积核卷积层，将数据维度从6000降到1500。
        """
...
if __name__ == "__main__":
    ...
    model_1 = base_MCNet(frame_length=1500, num_classes=16)
    summary(model_1, input_size=(1, 2, 6000), batch_size=1, device="cpu")
