...
class LCNet(nn.Module):
    def __init__(self, num_classes, n_neuron, n_mobileunit):
        ...
        self.global_max_pool = nn.AdaptiveMaxPool1d(1)
        self.fc1 = nn.Linear(1216, num_classes)
...
class base_LCNet(nn.Module):
    def __init__(self, num_classes, n_neuron, n_mobileunit):
        ...
        self.lamda = torch.nn.Parameter(torch.Tensor(np.ones(1216)))
    ...
    def forward(self, x):
        ...
        s_f = int(1216-torch.sum(lamda == 0))
        print('稀疏特征数量 = ', s_f)
        x *= lamda
...
if __name__ == '__main__':
    ...
    model_1 = base_LCNet(num_classes=10, n_neuron=32, n_mobileunit=7)
    Total_params, Params_size = summary(model_1, input_size=(2, 4800), batch_size=1, device="cpu")
